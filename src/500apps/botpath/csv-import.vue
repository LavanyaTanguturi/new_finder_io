          ...
          <table :class="tableClass">
            ...
            <tbody>
              <tr v-for="(column, key) in firstRow" :key="key">
                <td>{{ column }}</td>
                <td>
                  <select v-model="map[key]">
                    <option :value="null">Ignore</option>

                    <optgroup v-for="(bot, key) in bots" :label="bot.id" :key="key">
                      <option v-for="(field, key) in getBotStepOptions(bot)" :key="key" :value="key">{{
                        field.label
                      }}</option>
                    </optgroup>
                  </select>
...
// https://raw.githubusercontent.com/jgile/vue-csv-import/master/src/components/VueCsvImport.vue
...
export default {
  ...
  methods: {
    getBotStepOptions(step) {
      console.log(step);
      // Get Bots
      // Iterate
      // Get keys
      return Object.keys(step)
        .filter((item) => ["url", "input", "expression"].includes(item))
        .map((item) => ({ label: item, key: "later" }));
    },
    submit() {
      const _this = this;
